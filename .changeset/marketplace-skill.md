...
- **`marketplace init --codex` writes an artifact Codex does not read.** A fixture carrying only `.agents/plugins/<name>.json` is rejected as an unsupported manifest. Tracked in #42; until it is fixed the skill routes Codex users to `--claude`, which is the file Codex actually reads.
