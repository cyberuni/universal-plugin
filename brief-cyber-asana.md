# Mission: upgrade Changesets CLI v2 -> v3 in cyberuni/cyber-asana

## Repo
You are in a git worktree of cyberuni/cyber-asana, branched off its default branch. The primary checkout is at
`/home/unional/code/cyberuni/cyber-asana` — do NOT reach into it, do not touch its working tree, and do not commit to the default branch.
...
cyberuni/cyber-asana calls that shared workflow and is still on CLI v2, so its release job goes red on the next push to
...
- Do not fix any other repo — you own cyberuni/cyber-asana only.
- Do not merge your PR. That is the Council's call.

## Reporting
`npx cyberlegion@0.3.1 mail send --to operator --subject "cyber-asana: <one line>" --body "<PR url or BLOCKED+why, checks status, anything unusual>"`
