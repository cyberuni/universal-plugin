...
call an in-place update, not a duplicate. Keep the pin current with the `upgrade-plugin` skill, which
bumps every `npx`/`upx universal-plugin@<version>` reference across a project at once.
